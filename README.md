...
- Untuk membuat program berjalan terus-menerus (otomatis) di background process, maka diperlukan `daemon` dalam sourcecode nya.
...
#### [Source Code](https://github.com/fandykun/SoalShift_modul2_B05/blob/master/soal3/soal3.c)
### Penjelasan
- Untuk mengekstrak file zip, maka perlu melakukan `exec` dengan perintah `unzip`
- Untuk mengetahui lokasi `execv`nya, dapat dengan mengetikkan
```bash
which unzip
```
- Untuk menyimpan daftar file keseluruhan dengan menggunakan `ls`, karena diminta yang memiliki ekstensi `.txt` maka diperlukan proses filter, dalam hal ini dapat menggunakan `ls` yang di pipe ( `|` )dengan  `grep` yang terdapat di linux.
...
#### [Sourcecode](https://github.com/fandykun/SoalShift_modul2_B05/tree/master/soal5)
### Penjelasan A
- Untuk mengetahui waktu yang dilakukan saat ini, maka perlu memanfaatkan library `time.h`
- Untuk membaca file lognya, perlu me-load file tersebut dengan `fopen("/var/log/syslog", "r")`, menggunakan r karena hanya perlu read dari file tersebut.
- Karena soal diminta untuk menjalankan setiap menit dan diminta per 30 menit membuat folder, maka perlu menghitung sleeptimenya hingga menit ke-30.
- Dapat dengan memanfaatkan `snprintf` untuk menyimpan datetime dan file lognya.

### Penjelasan B
- Untuk mengetahui PID dari soal A, maka dalam program soal A perlu melakukan save kode PID nya.
- Hasil file tersebut yang kemudian di load diprogram soal B.
- Dengan menggunakan `fscanf` maka akan mendapat isi dari file tersebut.
- Kemudian kill isifile tersebut dengan fungsi `kill(pid, signal)`, dimana kill signal defaultnya yaitu `SIGTERM`